...
import { Trophy, Medal, Star, User, Shield, Brain, Gamepad2, HelpCircle } from 'lucide-react';
...
const Ranking: React.FC<RankingProps> = ({ members }) => {
  ...
  const getSortedData = () => {
    ...
    if (tab === 'quiz') return data.sort((a, b) => calculateSpecific(b, 'quiz') - calculateSpecific(a, 'quiz'));
    if (tab === 'memory') return data.sort((a, b) => calculateSpecific(b, 'memoryGame') - calculateSpecific(a, 'memoryGame'));
    if (tab === 'specialty') return data.sort((a, b) => calculateSpecific(b, 'specialtyGame') - calculateSpecific(a, 'specialtyGame'));
    if (tab === 'threeclues') return data.sort((a, b) => calculateSpecific(b, 'threeCluesGame') - calculateSpecific(a, 'threeCluesGame'));
  ...
  const getPoints = (m: Member | null) => {
    if (!m) return 0;
    if (tab === 'quiz') return calculateSpecific(m, 'quiz');
    if (tab === 'memory') return calculateSpecific(m, 'memoryGame');
    if (tab === 'specialty') return calculateSpecific(m, 'specialtyGame');
    if (tab === 'threeclues') return calculateSpecific(m, 'threeCluesGame');
