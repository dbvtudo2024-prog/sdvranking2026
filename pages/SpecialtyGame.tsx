      ...
      <div className="fixed inset-0 bg-black/40 backdrop-blur-md z-[100] flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-sm rounded-[3.5rem] p-8 sm:p-12 text-center space-y-8 animate-in zoom-in-95 duration-300">
          ...
          <div className="space-y-3">
            {DIFFICULTIES.map(d => (
              <button key={d.name} onClick={() => setDifficulty(d)} className="w-full bg-slate-50 hover:bg-blue-50 border border-slate-100 p-5 rounded-2xl flex justify-between items-center transition-all active:scale-95 group">
                <span className="font-black text-slate-700 uppercase tracking-widest text-sm group-hover:text-[#0061f2]">{d.name}</span>
                <div className="flex items-center gap-2"><Timer size={14} className="text-slate-300" /><span className="bg-[#FFD700] text-[#003366] px-4 py-1.5 rounded-full font-black text-xs">{d.time}s</span></div>
              </button>
            ))}
          </div>
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-500">
      <div className="flex items-center justify-between mb-8">
        <button onClick={onBack} className="p-3 bg-slate-100 rounded-2xl text-slate-400 hover:text-red-500 transition-colors"><ArrowLeft size={20} /></button>
        <div className="flex items-center gap-2 px-6 py-2 bg-blue-50 text-[#0061f2] rounded-full border border-blue-100 shadow-sm">
          <Timer size={18} strokeWidth={3} /><span className="font-black text-xl font-mono">{timeLeft}s</span>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center space-y-12">
        <div className="relative">
          <div className="w-48 h-48 rounded-full bg-white shadow-2xl flex items-center justify-center p-8 border-4 border-slate-50 relative overflow-hidden group">
            ...
            {showFeedback === 'correct' && <div className="absolute inset-0 bg-green-500/90 flex items-center justify-center text-white animate-in fade-in zoom-in duration-300"><Check size={80} strokeWidth={4} /></div>}
            {showFeedback === 'wrong' && <div className="absolute inset-0 bg-red-500/90 flex items-center justify-center text-white animate-in fade-in zoom-in duration-300"><X size={80} strokeWidth={4} /></div>}
          </div>
          <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 bg-slate-800 text-white px-6 py-1.5 rounded-full font-black text-[10px] uppercase tracking-[0.2em] shadow-lg whitespace-nowrap">QUESTÃO {currentIdx + 1} / {gameQuestions.length}</div>
        </div>
        <div className="grid grid-cols-1 gap-3 w-full max-w-sm">
          {currentQ.options.map((opt: any, idx: number) => (
            <button key={idx} disabled={!!showFeedback} onClick={() => handleAnswer(opt.name)} className={`w-full p-4 rounded-2xl border-2 font-black text-xs sm:text-sm uppercase tracking-widest transition-all active:scale-95 shadow-sm ${showFeedback === 'correct' && opt.name === currentQ.name ? 'bg-green-500 border-green-500 text-white shadow-green-500/20' : showFeedback === 'wrong' && opt.name !== currentQ.name ? 'bg-slate-50 border-slate-100 text-slate-300 opacity-50' : showFeedback === 'wrong' && opt.name === currentQ.name ? 'bg-green-500 border-green-500 text-white shadow-green-500/20' : 'bg-white border-slate-100 text-slate-600 hover:border-blue-200 hover:bg-blue-50/30'}`}>{opt.name}</button>
          ))}
        </div>
      </div>
    </div>
