  ...
  const handleMigrateData = async () => {
    ...
    try {
      for (const table of tables) {
        ...
        // 2. Prepare data for insert: filter out UUIDs for non-relational tables to let DB generate bigints
        const dataToInsert = oldData.map(item => {
          ...
          const isRelationalTable = ['members', 'users', 'messages', 'announcements'].includes(table);
...
{`-- 1. CORREÇÃO DE COLUNAS EXISTENTES
...
CREATE TABLE IF NOT EXISTS devotionals (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(), title TEXT, content TEXT, link TEXT, scheduled_for TIMESTAMP WITH TIME ZONE);
CREATE TABLE IF NOT EXISTS quizzes (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(), user_id UUID, score INTEGER, category TEXT);
CREATE TABLE IF NOT EXISTS who_am_i_questions (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, content TEXT, correct_answer TEXT, clues JSONB);
...
CREATE TABLE IF NOT EXISTS "EspecialidadesDBV" (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "Nome" TEXT, "Imagem" TEXT, "Categoria" TEXT);
...
-- 3. GARANTIR COLUNAS ESPECÍFICAS
...
-- 4. HABILITAR RLS E CRIAR POLÍTICAS DE ACESSO TOTAL (MODO ABERTO)
-- Rode este bloco para liberar o acesso as tabelas se estiver dando erro de permissão
DO $$ 
DECLARE
    t TEXT;
    tables TEXT[] := ARRAY['devotionals', 'quizzes', 'who_am_i_questions', 'EspecialidadesDBV', 'game_configs', 'game_assets', 'members', 'messages', 'announcements', 'specialty_studies', 'scrambled_verses', 'puzzle_images', 'quiz_questions', 'three_clues_questions', 'users', 'desbravadores', 'especialidades'];
BEGIN
    FOREACH t IN ARRAY tables LOOP
        ...
        EXCEPTION WHEN OTHERS THEN 
            RAISE NOTICE 'Não foi possível habilitar RLS na tabela %', t;
        END;

        -- Tenta criar política de acesso público total
        BEGIN
            EXECUTE 'DROP POLICY IF EXISTS "Public Access" ON "' || t || '"';
            EXECUTE 'DROP POLICY IF EXISTS "Enable all for all users" ON "' || t || '"';
            EXECUTE 'CREATE POLICY "Public Access" ON "' || t || '" FOR ALL USING (true) WITH CHECK (true)';
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'Não foi possível criar política na tabela %', t;
        END;
    END LOOP;
END $$;`}
                  </pre>
