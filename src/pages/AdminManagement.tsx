  ...
  const handleMigrateData = async () => {
    ...
    try {
      for (const table of tables) {
        ...
        const { error: insertError } = await supabase.from(table).upsert(oldData);
...
-- 3. HABILITAR RLS E CRIAR POLÍTICAS DE ACESSO PÚBLICO (UNRESTRICTED)
-- Execute para todas as tabelas que estiverem com erro
DO $$ 
DECLARE
    t TEXT;
    tables TEXT[] := ARRAY['devotionals', 'quizzes', 'who_am_i_questions', 'EspecialidadesDBV', 'game_configs', 'game_assets', 'members', 'messages', 'announcements', 'specialty_studies', 'scrambled_verses', 'puzzle_images', 'quiz_questions'];
BEGIN
